...
A modular GitHub Action that collects rich pull-request context, hands it to a review tool (mock LLM for local runs or a CLI), and maps the structured response back into GitHub reviews, comments, and step summaries.
...
- **Tool adapters (`internal/action/tool.go`)** – pluggable “review brains”: `mock` (in-process heuristic) or `cmd`. They all speak the same `ReviewResult` contract.
...
| Input | Purpose |
| ----- | ------- |
| `tool_mode` | `mock` (default) or `cmd` |
| `tool_cmd`, `tool_args_json`, `working_directory` | CLI tool wiring |
| `trigger_phrase`, `label_trigger`, `assignee_trigger` | Control when the action actually runs (default trigger phrase is empty for always-on) |
...
## Integrating a Real Review Tool

Switch `tool_mode` to `cmd` once your backend is ready:
...
3. **End-to-end in GitHub** – push a branch with the action, open a PR, and (if you set a trigger phrase) mention it in the PR body or a comment. Check the PR timeline, review tab, and job summary for the mock output.
...
- `.github/workflows/lint.yml` runs on pushes to `main`, tags, and every pull request. The `golangci-lint` job uses `golangci/golangci-lint-action@v9` against this repo's config.
- `.github/workflows/publish-image.yml` builds and publishes the action image on `main` and on semver tags (for example `v1.2.3`, plus `v1` and `v1.2` tags).
- Tags pushed with the `svu` helpers automatically benefit from fetching the full tag history in CI, so the workflows keep `fetch-depth: 0` to make those commands consistent locally and in automation.
- Security coverage comes from `codeql-analysis` and `dependency-scanning` workflows. They run weekly plus on every PR/push to `main`, covering CodeQL and GitHub dependency review.

## Publishing Checklist

1. Build the action container: `docker build -t agent-action .`
2. Push to a new repository, tag a release (`v1.2.3`), and reference it via `uses: <owner>/<repo>@v1`.
3. Gradually migrate consumers from mock to `cmd` mode once your review tool is ready.
