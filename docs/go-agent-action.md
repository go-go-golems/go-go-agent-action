...
The Go Agent Code Review Action packages GitHub pull-request context into a single payload, forwards it to a review tool (mock LLM or CLI), and publishes the response back to the pull request. This document explains how the action is structured, how to configure it, and how to validate it locally and in CI.
...
- **`internal/action/tool.go`** defines pluggable adapters: `MockTool` for deterministic local runs and `CommandTool` for local executables. Each adapter returns a `ReviewResult` with summary markdown, review comments, and decision metadata.
...
- **Trigger controls** ensure reviews only happen when requested: `trigger_phrase` (default empty for always-on), `label_trigger`, and `assignee_trigger`.
...
- **Tool selection** swaps the review backend: `tool_mode` (`mock` or `cmd`). Command mode uses `tool_cmd`, `tool_args_json`, and `working_directory`.
...
2. **Replace the review brain** – Implement a CLI that accepts the `PRContext` JSON. Swap `tool_mode` defaults in `internal/action/config.go` or change `buildTool` in `cmd/agent-action/main.go` to point at your binary.
...
- **Always-on reviews** – Create `.github/workflows/go-agent-review.yml` that triggers on `pull_request` events, sets `pull-requests: write` permissions, runs `actions/checkout`, and invokes the action with `tool_mode` of your choice. (Default trigger phrase is empty, so it runs without mention gating unless you set it.)
- **On-demand mentions** – Add a second workflow listening to `issue_comment` events with a guard such as `contains(github.event.comment.body, '@agent')`, and set `trigger_phrase: "@agent"` in the action inputs. This keeps noise low while letting developers request a review when they are ready.
...
Test both workflows by opening a PR and (if you enabled a trigger phrase) leaving a comment that contains it. The mock reviewer responds instantly, giving you confidence before pointing the action at a real LLM backend.
...
- **Enhanced tooling** – Implement additional tools in `internal/action/tool.go`, such as a multi-agent coordinator that fans out to several processes before aggregating comments.
...
The action slots into standard PR workflows. Include `actions/checkout` with `persist-credentials: false`, then invoke the action. With the mock tool you can iterate safely; switching to `tool_mode: cmd` later keeps the surrounding workflow identical.
...
2. Push the repository to GitHub and create a semver release tag (for example `v1.2.3`). The publish-image workflow will tag `v1.2.3`, `v1.2`, and `v1` in GHCR.
